...
########################################################################################################
# def LeafHumidityStomaResis(leaf_psi, veg_tempk, ceair, can_press, veg_esat, rb, gstoma, ft):
#
#     if (leaf_psi < 0.0):
#         lwp_star = np.exp(k_lwp[ft] * leaf_psi * molar_mass_water / (rgas_J_K_mol * veg_tempk))
#     else:
#         lwp_star = 1.0
#
#
...
def get_data(df_input):
    ...
    can_co2_ppress  = 0.00039 * can_press
    ...
    #ft              = torch.IntTensor([0 if c3c4_pathway[i] == "C3" else 1 for i in range(len(c3c4_pathway))])
    ...
    return (    can_press               , can_o2_ppress  , can_co2_ppress, veg_tempk, parsun_lsl   ,\
...
def pre_fates(forcing,vcmax25top_ft, btran):
    ...
    can_press               , can_o2_ppress  , can_co2_ppress, veg_tempk, parsun_lsl   ,\
    veg_esat                , c3c4_path_index, mm_kco2       , mm_ko2   ,co2_cpoint    ,\
    cf                      , gb_mol         , ceair         , stomatal_intercept_btran,\
