...
def readfile(file, pft_lst, to_frac = True):
    ...
    # This argumnent is very tricky so needs to make sure if they were already cinverted to fractions in your dtaaset or not.
...
########################################################################################################################
def onehot_col(df, colname):
    # Description: A function to one hot encode a categorical column by name
    # Inputs:
    # df     : The dataset dataframe
    # colname: The categorical column to be one hot encoded

    one_hot_PFT = pd.get_dummies(df[colname])
    df          = df.reset_index(drop = True);
    one_hot_PFT = one_hot_PFT.reset_index(drop = True)
    df          = df.join(one_hot_PFT)
    return df
