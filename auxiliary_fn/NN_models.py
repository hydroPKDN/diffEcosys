...
class Network_relu(nn.Module):
    # Network of three layers : Input, hidden, and output layer
    # RELU activation functions are used for both hidden and output layers
    def __init__(self, In, nh, out):
        super().__init__()
...
class Network_sig(nn.Module):
    ...
    def forward(self, x_cont):
        ...
        return x
#####################################################################################################################
class Network_sig_ot(nn.Module):
    # A more than the previous versions that can include different numbers of layers as indicated by the user.
...
class Network_relu_sig(nn.Module):
    ...
    # RELU activation functions are used for the hidden layers and sigmoid for the output layer
    def __init__(self, n_cont, out_sz, layers, p = 0.4):
        super().__init__()
...
#####################################################################################################################
class TabularModel_gelu_sig(nn.Module):
    # A more flexible than the previous versions that can include different numbers of layers as indicated by the user.
    # It can also digest categorical and continuous inputs simultaneously through the embedding layer

    # GELU activation functions are used for the hidden layers and Sigmoid activation functions for the output layer
    def __init__(self, emb_szs, n_cont, out_sz, layers, p = 0.4):
        super().__init__()
...
#####################################################################################################################
class TabularModel_relu_sig(nn.Module):
    # A more flexible than the previous versions that can include different numbers of layers as indicated by the user.
    # It can also digest categorical and continuous inputs simultaneously through the embedding layer

    # RELU activation functions are used for the hidden layers and Sigmoid activation functions for the output layer
    def __init__(self, emb_szs, n_cont, out_sz, layers, p = 0.4):
        super().__init__()
    ...
    def forward(self, x_cat, x_cont):
        ...
        return x
#####################################################################################################################
class TabularModel_sig_ot(nn.Module):
    # A more flexible than the previous versions that can include different numbers of layers as indicated by the user.
    # It can also digest categorical and continuous inputs simultaneously through the embedding layer
    ...
    def forward(self, x_cat, x_cont):
        ...
        return x
#####################################################################################################################
class TabularModel_relu_ot(nn.Module):
    # A more flexible than the previous versions that can include different numbers of layers as indicated by the user.
    # It can also digest categorical and continuous inputs simultaneously through the embedding layer

    # RELU activation functions are used for the hidden layers and NONE for the output layer
    def __init__(self, emb_szs, n_cont, out_sz, layers, p = 0.4):
        super().__init__()
...
# class Network_Sig_DD(nn.Module):
#     def __init__(self, In, nh, out, p = 0.4):
#         super().__init__()
#
#         self.hidden = nn.Linear(In, nh)
#         self.output = nn.Linear(nh, out)
