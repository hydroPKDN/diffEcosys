...
def rtsafe(x0, G, J, lb, ub, settings, doPrint=False):
    ...
    # x: [nb]
    iter=0; ftol=1e3; xtol=1e4; prec=1e-10
    nx = 1 if x0.ndim==1 else x0.shape[-1]
    x1 = torch.zeros_like(x0)+lb
    x2 = torch.zeros_like(x0)+ub
    alphaScalar = settings["alpha"];maxiter=settings["maxiter"];ftol_crit=settings["ftol"];xtol_crit=settings["xtol"]
    ...
    maskNull = fl*fh>0
    dxOld = x2-x1; dx=dxOld.clone(); fOld = f.clone();
    ...
    for iter in range(maxiter):
        ...
        iter +=1
        if doPrint:
            print(f"iter={iter}, x= {float(x[1])}, y= {float(f[1])}, dfdx= {float(dfdx[1])}, xtol= {xtol}, ftol= {ftol}")
...
class Model_equations(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #mask   = mask.type(torch.uint8)
        #gs_mol = mask * stomatal_intercept_btran_Med + (1 - mask) * quadratic_max(aquad, bquad, cquad)
        #gs_mol = quadratic_max(aquad, bquad, cquad)  #****
        #sol 1
        #gs_mol[torch.where(torch.isnan(gs_mol))] = stomatal_intercept_btran_Med[torch.where(torch.isnan(gs_mol))] #not working and give nan as well where the nan appear in dfdx
...
def testJacobian():
    ...
    J2 = Jacobian(mtd="batchScalarJacobian_FD", func=f, settings={"dx":1e-2,"bounds":(200000.0, 1e6)})
    vG = tensorNewton(f,J1)
    x0 = torch.tensor([483002.0260870451],requires_grad=True) # FD can be run in eval mode
    x  = vG(x0)

    vG = tensorNewton(f,J2)
    x0 = torch.tensor([483002.0],requires_grad=True)
    x  = vG(x0)

    vG = tensorNewton(f,J1,mtd="rtsafe",lb=182006,ub=682006,)
    x0 = torch.tensor([172006.0,483002.0,782006.0],requires_grad=False)
    x  = vG(x0)
    print(x)
